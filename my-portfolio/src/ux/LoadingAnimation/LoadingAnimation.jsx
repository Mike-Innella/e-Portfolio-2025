...
export function LoaderBackground() {
  ...
  return (
    <div className="loader-bg" ref={bgRef} />
  );
}
  ...
  useEffect(() => {
    const animate = () => {
      ...
      for (let ring = 1; ring <= 4; ring++) {
        ...
        rotationAngles.current[ring] = (elapsed * degreesPerMs) % 360;
      }
      
      // Update secondary angle for ring 4 (slightly slower)
      secondaryAngles.current[4] = (elapsed * (360 / (rotationSpeeds[4] * 1.5))) % 360;
  ...
  const getRingStyle = (ringNumber) => {
    ...
    switch (ringNumber) {
      ...
      case 4: {
        ...
        transform = `translate(-50%, -50%) rotateZ(${spin}deg) rotateX(${secondarySpin/4}deg) rotateY(${base.y}deg)`;
        break;
      }
      default:
